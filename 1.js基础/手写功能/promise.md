...
function Promise(excutor) {
  ...
  self.callbacks = [] // [{ onResolved(){}, onRejected(){} }, { onResolved(){}, onRejected(){} }]
...
``` js
const selfPromise = window.Promise
new selfPromise((reslove, reject) => {
  reslove('cheng')
}).then((res) => {
  console.log('成功', res); // 成功 cheng
}).then((res) => {
  console.log('成功', res); // 成功 undefined
}).then((res) => {
  console.log('成功', res); // 成功 undefined
})
```